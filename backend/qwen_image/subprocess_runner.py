...
from backend.qwen_image.pipeline import (
        generate_img2img_in_process,
        generate_inpaint_in_process,
        generate_text2img_in_process,
    )
from backend.qwen_image.subprocess_io import deserialize_params_from_subprocess
...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 2:
        print("Usage: python -m backend.qwen_image.subprocess_runner <input-json> <output-json>", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if operation not in dispatch:
            raise ValueError(f"Unsupported Qwen-Image subprocess operation: {operation}")
        ...
        try:
            ...
            print("Qwen-Image subprocess cleanup failed:", file=sys.stderr)
