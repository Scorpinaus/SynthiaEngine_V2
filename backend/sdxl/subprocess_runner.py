...
def main(argv: list[str] | None = None) -> int:
    ...
    if len(args) != 2:
        print("Usage: python -m backend.sdxl.subprocess_runner <input-json> <output-json>", file=sys.stderr)
        return 2
    ...
    try:
        ...
        if operation not in dispatch:
            raise ValueError(f"Unsupported SDXL subprocess operation: {operation}")
        ...
        try:
            ...
            print("SDXL subprocess cleanup failed:", file=sys.stderr)
