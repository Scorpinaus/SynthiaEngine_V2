...
function collectFluxImg2ImgPresetSettings() {
    return {
...
async function applyFluxImg2ImgPresetSettings(settings) {
...
window.PresetPanel?.init({
    apiBase: API_BASE,
    family: "flux",
    taskType: "flux.img2img",
    collectSettings: collectFluxImg2ImgPresetSettings,
    applySettings: applyFluxImg2ImgPresetSettings,
});
if (window.WorkflowCatalog?.load) {
    void window.WorkflowCatalog
        .load(API_BASE)
        .then(() => {
            window.WorkflowCatalog.applyDefaultsToForm("flux.img2img", {
                steps: "steps",
                cfg: "guidance_scale",
                width: "width",
                height: "height",
...
async function generateFluxImg2Img() {
    ...
    const defaults = catalog?.tasks?.["flux.img2img"]?.input_defaults ?? {};
    ...
    try {
        ...
        console.warn("Failed to run Flux img2img job:", error);
        gallery.setImages([]);
    }
}
