...
function collectZImageImg2ImgPresetSettings() {
    return {
...
async function applyZImageImg2ImgPresetSettings(settings) {
...
window.PresetPanel?.init({
    apiBase: API_BASE,
    family: "z-image",
    taskType: "z-image.img2img",
    collectSettings: collectZImageImg2ImgPresetSettings,
    applySettings: applyZImageImg2ImgPresetSettings,
});
if (window.WorkflowCatalog?.load) {
    void window.WorkflowCatalog
        .load(API_BASE)
        .then(() => {
            window.WorkflowCatalog.applyDefaultsToForm("z-image.img2img", {
                steps: "steps",
                cfg: "guidance_scale",
                width: "width",
                height: "height",
...
async function generateZImageImg2Img() {
    ...
    const defaults = catalog?.tasks?.["z-image.img2img"]?.input_defaults ?? {};
