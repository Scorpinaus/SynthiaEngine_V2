...
function collectZImagePresetSettings() {
    return {
...
async function applyZImagePresetSettings(settings) {
...
window.PresetPanel?.init({
    ...
    collectSettings: collectZImagePresetSettings,
    applySettings: applyZImagePresetSettings,
});
if (window.WorkflowCatalog?.load) {
    void window.WorkflowCatalog
        .load(API_BASE)
        .then(() => {
            window.WorkflowCatalog.applyDefaultsToForm("z-image.text2img", {
                steps: "steps",
                cfg: "guidance_scale",
                width: "width",
                height: "height",
