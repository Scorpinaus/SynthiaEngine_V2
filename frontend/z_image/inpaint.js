...
function collectZImageInpaintPresetSettings() {
    return {
        ...
        guidance_scale: WorkflowClient.readNumberValue("guidance_scale", 0.0),
...
async function applyZImageInpaintPresetSettings(settings) {
    ...
    setInputValue("guidance_scale", settings.guidance_scale);
...
loadModels();
loraPanelReady = window.LoraPanel?.init({ apiBase: API_BASE, family: "z-image" }) ?? Promise.resolve();
window.PresetPanel?.init({
    apiBase: API_BASE,
    family: "z-image",
    taskType: "z-image.inpaint",
    collectSettings: collectZImageInpaintPresetSettings,
    applySettings: applyZImageInpaintPresetSettings,
});
if (window.WorkflowCatalog?.load) {
    void window.WorkflowCatalog
        .load(API_BASE)
        .then(() => {
            window.WorkflowCatalog.applyDefaultsToForm("z-image.inpaint", {
                steps: "steps",
...
function resizeCanvasDisplay(image) {
    ...
    clearMask();
    imageInfo.textContent = `Image size: ${image.width} x ${image.height} (${Math.round(displayScale * 100)}% view)`;
}
...
function baseInput(inputs, defaults) {
    ...
    const guidance_scale = WorkflowClient.readNumberValue(
        "guidance_scale", defaults.guidance_scale ?? 0.0);
...
async function generateZImageInpaint() {
    ...
    try {
        ...
        console.warn("Failed to run Z-Image inpaint job:", error);
        gallery.setImages([]);
    }
}

window.generateZImageInpaint = generateZImageInpaint;
