...
function updateAdapterSummary() {
    ...
    setText(
        ...
        ipActive
            ? `Image prompt enabled${ipAdapter.hasReference ? " with reference image" : ""}.`
...
function initAdapterModal() {
    ...
    setText("adapter-modal-subtitle", "SDXL adapter stack");
...
function collectSdxlInpaintPresetSettings() {
    return {
        ...
        guidance_scale: WorkflowClient.readNumberValue("guidance_scale", 7.5),
...
async function applySdxlInpaintPresetSettings(settings) {
...
initAdapterModal();
ipAdapterPanelReady = Promise.resolve()
    .then(() => {
...
window.PresetPanel?.init({
    apiBase: API_BASE,
    family: "sdxl",
    taskType: "sdxl.inpaint",
    collectSettings: collectSdxlInpaintPresetSettings,
    applySettings: applySdxlInpaintPresetSettings,
});
